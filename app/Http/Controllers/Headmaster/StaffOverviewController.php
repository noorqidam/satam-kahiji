...
use App\Models\WorkItemFeedback;
...
{
    /**
     * Display staff overview with work item submissions
     */
    public function index(): Response
    {
        Gate::authorize('view-staff-overview');
        ...
        return Inertia::render('headmaster/staff-overview', [
            'teachers' => $teachers,
            'workItems' => $workItems,
            'stats' => $stats,
        ]);
    ...
    /**
     * Show detailed view of a specific teacher's work items
     */
    public function show(Staff $staff): Response
    {
        ...
        return Inertia::render('headmaster/teacher-detail', [
            'teacher' => $teacher,
        ]);
    }

    /**
     * Store feedback for a work item file
     */
    public function storeFeedback(Request $request)
    {
        Gate::authorize('provide-feedback');

        $request->validate([
            'teacher_work_file_id' => 'required|exists:teacher_work_files,id',
            'feedback' => 'required|string|max:1000',
            'status' => 'required|in:pending,approved,needs_revision',
        ]);

        $feedback = WorkItemFeedback::updateOrCreate(
            [
                'teacher_work_file_id' => $request->teacher_work_file_id,
                'reviewer_id' => Auth::id(),
            ],
            [
     ...
     */
    public function getFeedback(TeacherWorkFile $file)
    {
        Gate::authorize('view-staff-overview');
