...
interface TeacherStudentEditProps {
    student: Student;
    teacher: Teacher;
    userRole: string;
    extracurriculars: Array<{
...
export default function TeacherStudentEdit({ student, extracurriculars, recordOptions }: TeacherStudentEditProps) {
    ...
    const handlePhotoChange = (file: File | null) => {
        if (file) {
            ...
            if (!['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'].includes(file.type)) {
                toast({
                    title: t('student_form.validation.invalid_file_type'),
                    description: t('student_form.validation.select_image_file'),
                    variant: 'destructive',
                });
                return;
            }
            // Reset deletion flag when new file is selected
            setPhotoDeleted(false);
        } else {
            // Mark photo as deleted when file is null
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        // Add photo deletion flag if photo was deleted
        if (photoDeleted) {
            submitData.append('delete_photo', '1');
        }

        // Add method override for PUT request
        submitData.append('_method', 'PUT');

        router.post(route('teacher.students.update', student.id), submitData, {
            forceFormData: true,
            onSuccess: () => {
                toast({
                    title: t('common.success'),
                    description: t('student_form.messages.updated_successfully'),
                    variant: 'success',
                });
            },
            onError: (errors) => {
                ...
                toast({
                    ...
                    description: t('student_form.validation.check_form_errors'),
                    variant: 'destructive',
                });
            },
            onFinish: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('student_form.title.edit')} ${student.name}`} />
            ...
            <div className="space-y-6 px-4 sm:space-y-8 sm:px-6">
                ...
                <div className="border-b border-gray-200 pb-4 sm:pb-6 dark:border-gray-700">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900 sm:text-2xl dark:text-gray-100">{t('student_form.title.edit')}</h1>
                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {t('student_form.subtitle.update_info', { name: student.name })}
                        </p>
                    </div>
                </div>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        <Input
                                            ...
                                            title={t('student_form.messages.class_readonly')}
                                        />
                                        <p className="text-xs text-gray-500">⚠️ {t('student_form.messages.class_readonly')}</p>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <Select
                                        value={formData.graduation_year || 'not_set'}
                                        onValueChange={(value) => handleInputChange('graduation_year', value === 'not_set' ? '' : value)}
                                    >
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('student_form.placeholders.graduation_year')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="not_set">{t('student_form.options.not_set')}</SelectItem>
                            ...
                            <div>
                                ...
                                />
                                <p className="text-xs text-gray-500">{t('student_form.messages.extracurricular_note')}</p>
                        ...
                        <Button type="submit" disabled={isSubmitting}>
                            {isSubmitting ? (
                                ...
                                <>
                                    ...
                                    {t('student_form.actions.update_student')}
                                </>
                            )}
                        </Button>
                    </div>
                ...
                {recordOptions && <StudentRecordsTabs student={student} recordOptions={recordOptions} isEdit={true} />}
            </div>
        </AppLayout>
    );
}
